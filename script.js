...
// --- API LAYER ---
/**
 * Core API caller for Google Apps Script
 */
...
/**
 * Fetches all data from the database to sync state
 */
async function refreshData() {
    console.log("[SYNC] Refreshing data from database...");

    const tasksRes = await apiCall('getTasks');
    if (tasksRes && tasksRes.status === 'success') {
        state.tasks = tasksRes.data || [];
        console.log(`[SYNC] Loaded ${state.tasks.length} tasks.`);
    }
...
/**
 * Hybrid Sync: Updates local UI immediately then syncs to Sheet in background
 */
async function syncAction(action, payload) {
    console.log(`[SYNC] Executing ${action}...`, payload);
    ...
    if (action === 'addTask') {
        ...
        payload.task_id = newTask.task_id;
        localSuccess = true;
    }
    ...
    // If server assigned a different ID or we need to refresh to be sure
    if (action === 'addTask' && result.status === 'success') {
        // Optionally update the local ID if server generated one
        // refreshData(); 
    }
...
async function init() {
    ...
    // Initial UI render (using sample data while fetching)
    renderTasks();
...
async function renderTasks() {
    ...
    const dailyTasks = state.tasks
        .filter(t => getTaskDate(t.date) === state.selectedDate)
    ...
    dailyTasks.forEach(task => {
        ...
        card.innerHTML = `
            ...
            <div class="task-main" onclick="openEditModal('${task.task_id}')">
                ...
                ${task.objective ? `<div class="task-objective" style="font-size: 0.9rem; color: var(--text-muted); margin-bottom: 0.5rem; font-style: italic;">Target: ${task.objective}</div>` : ''}
                <div class="task-meta">
...
async function quickAddTask() {
    ...
    const payload = {
        ...
        category: 'Personal', // Default for quick add
        priority: 3,
        notes: ''
    };

    await syncAction('addTask', payload);
    quickInput.value = '';
}
...
async function toggleStatus(taskId) {
    ...
    await syncAction('updateTask', { task_id: taskId, status: nextStatus });
}
...
function openAddModal() {
    ...
    document.getElementById('taskTitle').value = quickInput.value.trim();
    ...
    const categories = state.settings.categories.split(',');
...
async function saveTaskFromModal() {
    ...
    if (state.editingTaskId) {
        payload.task_id = state.editingTaskId;
        await syncAction('updateTask', payload);
    } else {
        await syncAction('addTask', payload);
        quickInput.value = '';
    }
